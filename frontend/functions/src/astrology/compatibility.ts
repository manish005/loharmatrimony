...
import { KundaliResult } from "./engine";
...
const VASHYA_TABLE: Record<string, string[]> = {
  Aries: ["Leo", "Scorpio"],
  Taurus: ["Cancer", "Libra"],
  Gemini: ["Virgo"],
...
function calcTara(boyNak: number, girlNak: number): number {
  ...
  if (auspicious.includes(group)) return 3;
  if (group === 2 || group === 4 || group === 6) return 0;
  return 1.5; // partial
}
...
const PLANET_FRIENDSHIPS: Record<string, { friends: string[], enemies: string[], neutral: string[] }> = {
...
function getPlanetRelation(p1: string, p2: string): "friend" | "enemy" | "neutral" {
  if (p1 === p2) return "friend";
  const f = PLANET_FRIENDSHIPS[p1];
  if (!f) return "neutral";
  if (f.friends.includes(p2)) return "friend";
  if (f.enemies.includes(p2)) return "enemy";
  return "neutral";
}
...
function calcBhakoot(boyMoon: number, girlMoon: number): { score: number; dosha: boolean } {
...
function calcNadi(boyNadi: string, girlNadi: string): { score: number; dosha: boolean } {
  if (boyNadi === girlNadi) {
...
export function calculateCompatibility(
  ...
  // 4. YONI (4 points)
  const yoniScore = (YONI_COMPATIBILITY[boy.yoni]?.[girl.yoni]) ?? 2;
  factors.push({
    name: "Yoni",
    nameHindi: "योनि",
    maxPoints: 4,
  ...
  let grahaScore = 0;
  if (boyToGirlRel === "friend" && girlToBoyRel === "friend") grahaScore = 5;
  else if (boyToGirlRel === "friend" || girlToBoyRel === "friend") grahaScore = 4;
  else if (boyToGirlRel === "neutral" && girlToBoyRel === "neutral") grahaScore = 3;
  else if (boyToGirlRel === "neutral" || girlToBoyRel === "neutral") grahaScore = 1;
  else grahaScore = 0;
  ...
  // 6. GANA (6 points)
  const ganaScore = GANA_COMPATIBILITY[boy.gana]?.[girl.gana] ?? 0;
  factors.push({
    name: "Gana",
    nameHindi: "गण",
    maxPoints: 6,
